...
public class Juego {
    ...
    public void llamarPreguntas(){
        ...
        String sql = "WITH resp AS (SELECT r.id_pregunta, r.titulo_respuesta, r.es_correcta, ROW_NUMBER() OVER (PARTITION BY r.id_pregunta, r.es_correcta "
                + "ORDER BY r.id_respuesta) AS rn FROM Respuesta r), pivot AS (SELECT p.numero_portfolio, e.nombre, e.apellido, "
                + "p.titulo AS titulo_portfolio, pr.titulo_pregunta, MAX(CASE WHEN r.es_correcta = 1 THEN r.titulo_respuesta END) "
                + "AS respuestaCorrecta, MAX(CASE WHEN r.es_correcta = 0 AND r.rn = 1 THEN r.titulo_respuesta END) AS opcion2, "
                + "MAX(CASE WHEN r.es_correcta = 0 AND r.rn = 2 THEN r.titulo_respuesta END) AS opcion3, MAX(CASE WHEN r.es_correcta = 0 AND r.rn = 3 "
                + "THEN r.titulo_respuesta END) AS opcion4 FROM Portfolio p JOIN Estudiante e ON p.id_estudiante = e.id_estudiante JOIN Pregunta pr "
                + "ON p.id_portfolio = pr.id_portfolio JOIN resp r ON pr.id_pregunta = r.id_pregunta "
                + "GROUP BY p.id_portfolio, e.nombre, e.apellido, p.titulo, pr.titulo_pregunta) SELECT * FROM pivot;";
        try{
            ...
            while (rs.next()) {
                ...
                registro[0] = rs.getString("numero_portfolio");
    ...
    public String hacerPregunta(JLabel label_pregunta, JButton boton1,JButton boton2,JButton boton3,JButton boton4){
        ...
        pregunta_completa = "Segun el (Portfolio  " + datos[0] + ")  del/la compañero/a  (" 
                + datos[1] + ")  del tema  (" + datos[2] + ")" + "\n" +" Responda: " + datos[3];
        
        String textoConFormato = "<html><p style=\"width:500px; text-align: center;\">" 
                + pregunta_completa.replace("\n","<br>") + "</p></html>";
